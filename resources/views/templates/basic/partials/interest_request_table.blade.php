            ...
            @forelse ($interestRequests as $interest)
                <tr>
                    ...
                    <td>
                        ...
                        @else
                        {{-- {{ route('user.message.index', encrypt($interest->conversation->id ?? '')) }} --}}
                            <a class="icon-anchor btn--messenger" data-bs-toggle="tooltip" href="" title="@lang('Message')"><i class="las la-envelope"></i></a>
                        @endif
                    </td>
                </tr>
            @empty
