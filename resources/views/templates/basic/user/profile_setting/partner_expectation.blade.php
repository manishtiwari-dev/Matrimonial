     ...
     <div class="accordion-item partner-expectation">
         ...
         <div class="accordion-collapse collapse" id="panelsStayOpen-collapsePartnerExp" aria-labelledby="panelsStayOpen-partnerExp">
             <div class="accordion-body">
                 <form class="partner-expectation-form" action="" autocomplete="off" method="POST" class="needs-validation"  novalidate>
                     @csrf
                     ...
                     <div class="row gy-4">
                             ...
                             <div class="input--group">
                                 <select class="form-select form-control form--control" name="religion">
                                     ...
                                     @foreach ($religions as $religion)
                                         <option value="{{ $religion->name }}"  @selected(old('religion', @$user->partnerExpectation->religion) == $religion->name)>{{ __($religion->name) }}</option>
                                     @endforeach
                                 </select>
                        ...
                        <div class="col-sm-6">
                            <div class="input--group">
                                <select class="form-select form-control form--control" name="community">
                                    <option value="">@lang('Select One')</option>
                                    @foreach ($community as $commun)
                                        <option value="{{ $commun->name }}" @selected(old('community', @$user->partnerExpectation->community) == $commun->name)>{{ __($commun->name) }}</option>
                                    @endforeach
                                </select>
                                <label class="form--label">@lang('Community')</label>
                            </div>
                        </div>

                        <div class="col-sm-6">
                            <div class="input--group">
                                <select class="form-select form-control form--control" name="mother_tongue">
                                    <option value="">@lang('Select One')</option>
                                    @foreach ($motherTongue as $motherTon)
                                        <option value="{{ $motherTon->name }}" @selected(old('mother_tongue', @$user->partnerExpectation->mother_tongue) == $motherTon->name)>{{ __($motherTon->name) }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="input--group">
                                <select class="form-control form--control select2-auto-tokenize2" name="language[]" multiple="multiple">
                                        ...
                                        @foreach ($languages as $lan)
                                            <option value="{{ $lan->code }}" selected>{{ $language->code }}</option>
                                        @endforeach
                                </select>
                                <label class="form--label">@lang('Languages')</label>
                            </div>
